...
//#else
#define MQTT_CALLBACK_SIGNATURE void (*callback)(char*, char*, uint16_t)
#define GF(x) x	
//#endif
// TODO: _client->stop implementation
//#define CHECK_STRING_LENGTH(l,s) if (l+2+strnlen(s, bufferSize) > bufferSize) {_client->stop();return false;}
//#define CHECK_STRING_LENGTH(l,s) if (l+2+strnlen(s, bufferSize) > bufferSize) {return false;}
...
// setServer(IPAddress ip, uint16_t port);


/*remember
 setServer(uint8_t * ip, uint16_t port);
 setServer2(const char * domain, uint16_t port);
 setCallback(MQTT_CALLBACK_SIGNATURE);
 */
...
/*remember
 ...
 setSocketTimeout(uint16_t timeout);
*/
...
//bool  publish_P2(const char* topic, const uint8_t * payload, unsigned int plength, bool retained);
